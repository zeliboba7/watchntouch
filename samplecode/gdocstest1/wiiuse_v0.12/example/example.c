	...
	if (WIIUSE_USING_IR(wm)) {
		...
		for (; i < 4; ++i) {
			...
			if (wm->ir.dot[i].visible) {
				...
				XWarpPointer(dpy, None, root_window, 0, 0, 0, 0,500- wm->ir.dot[i].x,700- wm->ir.dot[i].y); // 4 kalbire noktalarını al sonra ona göre transform et !!
				XFlush(dpy);
			}
		}
...
int main(int argc, char** argv) {
	...
	// below three line is to control the mouse
	...
	/*printf("Please do the calibration\n");	
